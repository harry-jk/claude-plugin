...
# captain → logs/captain.md / teammate → logs/{session_id}.md
# 10초 이내 수정되지 않았으면 block

LOGS_DIR="$CLAUDE_PROJECT_DIR/.claude/voyage/logs"

# voyage 디렉토리가 없으면 스킵 (voyage 미사용 프로젝트)
[ ! -d "$LOGS_DIR" ] && exit 0
...
if [ "$SESSION_ID" = "$LEAD_SESSION" ]; then
  ...
  FILE_LABEL="logs/captain.md"
elif [ -f "$LOGS_DIR/${SESSION_ID}.md" ]; then
  STATUS_FILE="$LOGS_DIR/${SESSION_ID}.md"
  FILE_LABEL="logs/${SESSION_ID}.md"
else
  # captain도 아니고 teammate 파일도 없음 → voyage 무관 세션, 스킵
  exit 0
fi

# 파일이 없으면 block
if [ ! -f "$STATUS_FILE" ]; then
  jq -n --arg reason ".claude/voyage/${FILE_LABEL} 파일이 없습니다. 현재 작업 내용, 진행 상황, 다음 단계를 기록하세요." \
...
if [ "$FILE_AGE_SEC" -gt 10 ]; then
  jq -n --arg reason ".claude/voyage/${FILE_LABEL}가 ${FILE_AGE_SEC}초 전 수정됨 (10초 이내 필요). 현재 작업 내용, 진행 상황, 다음 단계를 지금 즉시 업데이트하세요." \
    '{"decision":"block","reason":$reason}'
  exit 0
fi
